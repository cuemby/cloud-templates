...
echo "Starting AlmaLinux CloudStack template preparation..."
...
if curl -m 5 -s ${PASSWORD_URL} > /dev/null; then
    ...
    if [ -n "$PASSWORD" ]; then
        echo "almalinux:$PASSWORD" | chpasswd
...
history -c
> /root/.bash_history
> /home/almalinux/.bash_history 2>/dev/null || true

# Clean DNF cache
dnf clean all
rm -rf /var/cache/dnf/*

echo "AlmaLinux CloudStack template preparation completed successfully!"