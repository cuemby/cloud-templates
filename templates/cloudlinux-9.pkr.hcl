...
variable "name" {
  type    = string
  default = "cloudlinux-9"
}

variable "url" {
  type    = string
  default = "https://downloads.cloudlinux.com/cloudlinux/9/isos/x86_64/CloudLinux-9.4-x86_64-boot.iso"
}

variable "iso_checksum" {
  type    = string
  default = "sha256:903641686cbd7caaaf9ddf38fbb2e01c083798866a05e38dbe4ff8748a8b3bf7"
}

variable "boot_command" {
  type    = list(string)
  default = ["<tab> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/kickstart/cloudlinux-9.ks<enter><wait>"]
}

source "qemu" "cloudlinux-9" {
  iso_url              = var.url
...
build {
  sources = ["source.qemu.cloudlinux-9"]
